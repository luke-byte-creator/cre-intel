...
function setCell(ws: ExcelJS.Worksheet, row: number, col: number, value: string | number | null | undefined, opts?: { font?: Partial<ExcelJS.Font>; fill?: ExcelJS.FillPattern; bold?: boolean; merge?: [number, number, number, number] }) {
  const cell = ws.getCell(row, col);
  cell.value = value ?? "";
  cell.font = opts?.font || (opts?.bold ? { ...dataFont, bold: true } : dataFont);
  if (opts?.fill) cell.fill = opts.fill;
  cell.border = thinBorder;
  if (opts?.merge) ws.mergeCells(opts.merge[0], opts.merge[1], opts.merge[2], opts.merge[3]);
}
...
function generateLeaseWorkbook(data: any, commission: any): ExcelJS.Workbook {
  ...
  ws.mergeCells(3, 2, 3, 3); setCell(ws, 3, 2, "RENEWAL" + (data.dealType === "Renewal" ? " ✓" : ""));
  ws.mergeCells(3, 4, 3, 5); setCell(ws, 3, 4, "EXTENSION" + (data.dealType === "Extension" ? " ✓" : ""));

  // Row 5: Space type
  setCell(ws, 5, 1, "RAW SPACE" + (data.spaceType === "Raw" ? " ✓" : ""));
  ws.mergeCells(5, 2, 5, 3); setCell(ws, 5, 2, "");
  ws.mergeCells(5, 3, 5, 5); setCell(ws, 5, 3, "IMPROVED SPACE" + (data.spaceType === "Improved" ? " ✓" : ""));

  // Row 6: Lease type + headers
  setCell(ws, 6, 1, "DIRECT LEASE:" + (data.leaseType === "Direct" ? " ✓" : ""));
  ws.mergeCells(6, 3, 6, 5); setCell(ws, 6, 3, "TERM:");
  setCell(ws, 6, 4, "DAY"); setCell(ws, 6, 5, "MONTH"); setCell(ws, 6, 6, "YEAR");
  ws.mergeCells(6, 8, 6, 9); setCell(ws, 6, 8, "(SUB)   LANDLORD", { fill: sectionFill, bold: true });
  ws.mergeCells(6, 10, 6, 11); setCell(ws, 6, 10, "(SUB)   TENANT", { fill: sectionFill, bold: true });

  // Row 7: Sublease + FROM
  ws.mergeCells(7, 1, 7, 2); setCell(ws, 7, 1, "SUBLEASE:" + (data.leaseType === "Sublease" ? " ✓" : ""));
  setCell(ws, 7, 3, "FROM:");
  ...
  ws.mergeCells(9, 1, 9, 2); setCell(ws, 9, 1, "RENEWAL OPTION");
  ws.mergeCells(9, 3, 9, 4); setCell(ws, 9, 3, "YES" + (data.renewalOption ? " ✓" : ""));
  ws.mergeCells(9, 5, 9, 6); setCell(ws, 9, 5, data.renewalDate ? `RENEWAL DATE: ${data.renewalDate}` : "");
  ws.mergeCells(10, 1, 10, 2); setCell(ws, 10, 1, "");
  ws.mergeCells(10, 3, 10, 4); setCell(ws, 10, 3, "NO" + (!data.renewalOption ? " ✓" : ""));

  // Row 11: Landlord/Tenant names
  ws.mergeCells(11, 1, 11, 3); setCell(ws, 11, 1, "(SUB)    LANDLORD NAME: ", { bold: true });
  ws.mergeCells(11, 4, 11, 6); setCell(ws, 11, 4, data.landlord?.name || "");
  ws.mergeCells(11, 7, 11, 8); setCell(ws, 11, 7, "(SUB)   TENANT NAME:  ", { bold: true });
  ws.mergeCells(11, 9, 11, 11); setCell(ws, 11, 9, data.tenant?.name || "");

  // Row 12: Contact names
  ws.mergeCells(12, 1, 12, 3); setCell(ws, 12, 1, "CONTACT NAME(REQUIRED): ");
  ws.mergeCells(12, 4, 12, 6); setCell(ws, 12, 4, data.landlord?.contactName || "");
  ws.mergeCells(12, 7, 12, 8); setCell(ws, 12, 7, "CONTACT NAME(REQUIRED): ");
  ws.mergeCells(12, 9, 12, 11); setCell(ws, 12, 9, data.tenant?.contactName || "");

  // Row 13: Phones
  setCell(ws, 13, 1, "PHONE:  ");
  setCell(ws, 13, 2, data.landlord?.phone || "");
  ws.mergeCells(13, 4, 13, 5); setCell(ws, 13, 4, "Fax/Email: ");
  setCell(ws, 13, 6, data.landlord?.email || "");
  setCell(ws, 13, 7, "PHONE:  ");
  setCell(ws, 13, 8, data.tenant?.phone || "");
  setCell(ws, 13, 9, "Fax/Email: ");
  ws.mergeCells(13, 10, 13, 11); setCell(ws, 13, 10, data.tenant?.email || "");
  ...
  ws.mergeCells(15, 1, 15, 6); setCell(ws, 15, 1, data.landlord?.address || "");
  ws.mergeCells(15, 7, 15, 11); setCell(ws, 15, 7, data.tenant?.address || "");
  ...
  ws.mergeCells(20, 1, 20, 6); setCell(ws, 20, 1, data.property?.address || "");
  ...
  ws.mergeCells(22, 7, 22, 11); setCell(ws, 22, 7, "DEAL DETAILS:", { fill: sectionFill, bold: true });
  ...
  ws.mergeCells(23, 7, 23, 8); setCell(ws, 23, 7, "Total SF of Leased Space:");
  ws.mergeCells(23, 9, 23, 11); setCell(ws, 23, 9, data.totalSF ?? "");

  ws.mergeCells(24, 7, 24, 8); setCell(ws, 24, 7, "Base Annual Rent PSF:");
  ws.mergeCells(24, 9, 24, 11); setCell(ws, 24, 9, data.baseAnnualRentPSF ?? "");

  ws.mergeCells(25, 7, 25, 8); setCell(ws, 25, 7, "Months of Free Rent:");
  ws.mergeCells(25, 9, 25, 11); setCell(ws, 25, 9, data.monthsFreeRent ?? "");

  ws.mergeCells(26, 7, 26, 8); setCell(ws, 26, 7, "Tenant Inducement:");
  ws.mergeCells(26, 9, 26, 11); setCell(ws, 26, 9, data.tenantInducementPSF ?? "");

  ws.mergeCells(27, 7, 27, 8); setCell(ws, 27, 7, "Taxes & Operating Costs:");
  ws.mergeCells(27, 9, 27, 11); setCell(ws, 27, 9, data.taxesOperatingCostsPSF ?? "");

  // Row 30: Invoice request
  ws.mergeCells(30, 7, 30, 8); setCell(ws, 30, 7, "INVOICE REQUEST DATE:");
  ws.mergeCells(30, 9, 30, 11); setCell(ws, 30, 9, "Immediate");

  // Row 31-32: Occupier industry label
  ws.mergeCells(31, 7, 32, 11); setCell(ws, 31, 7, "OCCUPIER INDUSTRY: CHOOSE APPLICABLE OCCUPIER FROM LIST", { bold: true });

  // Row 33: Occupier industry value
  ws.mergeCells(33, 7, 33, 8); setCell(ws, 33, 7, "Occupier Industry: ");
  ws.mergeCells(33, 9, 33, 11); setCell(ws, 33, 9, data.occupierIndustry || "");

  // Row 34: Building classification
  ws.mergeCells(34, 7, 34, 8); setCell(ws, 34, 7, "Building Classification:");
  ws.mergeCells(34, 9, 34, 11); setCell(ws, 34, 9, data.buildingClassification || "");

  // Row 35: Space use
  ws.mergeCells(35, 7, 35, 8); setCell(ws, 35, 7, "Space Use:");
  ws.mergeCells(35, 9, 35, 11); setCell(ws, 35, 9, data.spaceUse || "");

  // Row 36: Reason for transaction
  ws.mergeCells(36, 7, 36, 8); setCell(ws, 36, 7, "Reason for Transaction:");
  ws.mergeCells(36, 9, 36, 11); setCell(ws, 36, 9, data.reasonForTransaction || "");

  // Row 40: Commission header
  setMergedLabel(ws, 40, 1, 11, "COMMISSION CALCULATION FOR BILLING:", headerFill);

  // Row 41: Column headers
  ws.mergeCells(41, 1, 41, 2); setCell(ws, 41, 1, "TOTAL SQ. FT", { bold: true });
  setCell(ws, 41, 3, "@");
  ws.mergeCells(41, 4, 41, 5); setCell(ws, 41, 4, "$ RATE PSF", { bold: true });
  setCell(ws, 41, 6, "@");
  setCell(ws, 41, 7, "TERM", { bold: true });
  setCell(ws, 41, 8, "TOTAL", { bold: true });
  setCell(ws, 41, 9, "LEASE RATE:", { bold: true });
  ws.mergeCells(41, 10, 41, 11); setCell(ws, 41, 10, "TOTAL COMMISSION", { bold: true });
  ...
  for (let i = 0; i < Math.max(rentPeriods.length, 1); i++) {
    ...
    if (period) {
      ...
      ws.mergeCells(row, 1, row, 2); setCell(ws, row, 1, totalSF);
      setCell(ws, row, 3, "@");
      ws.mergeCells(row, 4, row, 5); setCell(ws, row, 4, period.rentPSF);
      setCell(ws, row, 6, "@");
      setCell(ws, row, 7, years);
      setCell(ws, row, 8, { formula: `A${row}*D${row}*G${row}` } as unknown as number);
      setCell(ws, row, 9, commRate);
      ws.mergeCells(row, 10, row, 11); setCell(ws, row, 10, { formula: `H${row}*I${row}` } as unknown as number);
    }
  }
  ...
  // Totals row
  ws.mergeCells(totalsRow, 1, totalsRow, 4); setCell(ws, totalsRow, 1, "TOTAL DEAL VALUE:", { bold: true });
  ws.mergeCells(totalsRow, 5, totalsRow, 6); setCell(ws, totalsRow, 5, { formula: `SUM(H42:H${lastCalcRow})` } as unknown as number);
  setCell(ws, totalsRow, 8, "TOTAL BILLING:", { bold: true });
  ws.mergeCells(totalsRow, 10, totalsRow, 11); setCell(ws, totalsRow, 10, { formula: `SUM(J42:K${lastCalcRow})` } as unknown as number);
  ...
  ws.mergeCells(hdrRow, 1, hdrRow, 3); setCell(ws, hdrRow, 1, "BRANCH NAME:", { bold: true });
  ws.mergeCells(hdrRow, 4, hdrRow, 5); setCell(ws, hdrRow, 4, "SALES PERSON:", { bold: true });
  setCell(ws, hdrRow, 6, "AMOUNT:", { bold: true });
  setCell(ws, hdrRow, 7, "Initials:", { bold: true });

  // Branch total row
  const brRow = hdrRow + 1;
  ws.mergeCells(brRow, 1, brRow, 3); setCell(ws, brRow, 1, "CBRE Saskatchewan");
  ...
  splits.forEach((s: { name: string; pct: number }, i: number) => {
    const r = brRow + 1 + i;
    ws.mergeCells(r, 1, r, 3); setCell(ws, r, 1, "CBRE Saskatchewan");
    ws.mergeCells(r, 4, r, 5); setCell(ws, r, 4, s.name);
  ...
  ws.mergeCells(totalRow, 1, totalRow, 5); setCell(ws, totalRow, 1, "CBRE & AGENTS TOTAL:", { bold: true });
  setCell(ws, totalRow, 6, { formula: `SUM(F${brRow}:F${lastSplitRow})` } as unknown as number);

  // Lease schedule section (right side)
  const lsRow = distRow;
  ws.mergeCells(lsRow, 8, lsRow, 11); setCell(ws, lsRow, 8, "LEASE SCHEDULE PER TERM AS PER OTL:", { fill: sectionFill, bold: true });
  ...
  ws.mergeCells(noteRow, 1, noteRow + 2, 6);
...
function generateSaleWorkbook(data: any, commission: any): ExcelJS.Workbook {
  ...
  // Row 1: Title
  ws.mergeCells(1, 1, 1, 8); setCell(ws, 1, 1, "SALE TRADE RECORD", { fill: headerFill, bold: true });
  ws.mergeCells(1, 9, 1, 11); setCell(ws, 1, 9, "Deal #", { bold: true });

  // Row 2: FINTRAC
  ws.mergeCells(2, 1, 2, 2); setCell(ws, 2, 1, "FINTRAC REPRESENTATION:", { bold: true });
  ws.mergeCells(2, 3, 2, 5); setCell(ws, 2, 3, "Vendor:  " + (data.fintracRepresentation === "Vendor" || data.fintracRepresentation === "Both" ? "✓" : ""));
  ws.mergeCells(2, 6, 2, 11); setCell(ws, 2, 6, "BUYER: " + (data.fintracRepresentation === "Buyer" || data.fintracRepresentation === "Both" ? "✓" : ""));

  // Row 3-4: Asset/Property Manager
  ws.mergeCells(3, 1, 4, 2); setCell(ws, 3, 1, "ASSET/PROPERTY MANAGER", { bold: true });
  ws.mergeCells(3, 3, 4, 8); setCell(ws, 3, 3, data.assetPropertyManager || "");
  setCell(ws, 3, 10, "BUYER"); setCell(ws, 3, 11, "VENDOR");

  // Row 5-7: Beneficial Owner + Engaged/Paid By
  ws.mergeCells(5, 1, 7, 2); setCell(ws, 5, 1, "BENEFICIAL OWNER (up to three)", { bold: true });
  ws.mergeCells(5, 3, 7, 7); setCell(ws, 5, 3, data.beneficialOwner || "");
  ws.mergeCells(5, 8, 5, 9); setCell(ws, 5, 8, "CBRE Engaged By:");
  setCell(ws, 5, 10, data.engagedBy === "Buyer" ? "✓" : "");
  setCell(ws, 5, 11, data.engagedBy === "Vendor" ? "✓" : "");
  ws.mergeCells(6, 8, 6, 9); setCell(ws, 6, 8, "CBRE Paid BY:");
  setCell(ws, 6, 10, data.paidBy === "Buyer" ? "✓" : "");
  setCell(ws, 6, 11, data.paidBy === "Vendor" ? "✓" : "");

  // Row 8: Vendor / Purchaser
  setCell(ws, 8, 1, "VENDOR: ", { bold: true });
  ws.mergeCells(8, 2, 8, 5); setCell(ws, 8, 2, data.vendor?.name || "");
  setCell(ws, 8, 6, "PURCHASER: ", { bold: true });
  ws.mergeCells(8, 7, 8, 11); setCell(ws, 8, 7, data.purchaser?.name || "");

  // Row 9: Contacts
  setCell(ws, 9, 1, "Contact Name: ");
  ws.mergeCells(9, 2, 9, 5); setCell(ws, 9, 2, data.vendor?.contactName || "");
  setCell(ws, 9, 6, "Contact Name: ");
  ws.mergeCells(9, 7, 9, 11); setCell(ws, 9, 7, data.purchaser?.contactName || "");

  // Row 10: Phones
  setCell(ws, 10, 1, "Phone:  ");
  ws.mergeCells(10, 2, 10, 3); setCell(ws, 10, 2, data.vendor?.phone || "");
  setCell(ws, 10, 4, "Fax: ");
  setCell(ws, 10, 6, "Phone:  ");
  ws.mergeCells(10, 7, 10, 8); setCell(ws, 10, 7, data.purchaser?.phone || "");
  setCell(ws, 10, 9, "Fax: ");

  // Row 11-12: Addresses
  ws.mergeCells(11, 1, 12, 4); setCell(ws, 11, 1, `Mailing Address:\n${data.vendor?.address || ""}`);
  setCell(ws, 11, 5, "SUITE #");
  ws.mergeCells(11, 6, 12, 9); setCell(ws, 11, 6, `Mailing Address:\n${data.purchaser?.address || ""}`);
  ws.mergeCells(11, 10, 11, 11); setCell(ws, 11, 10, "SUITE #");

  // Row 13-14: City/Province/Postal
  setCell(ws, 13, 1, "City:"); setCell(ws, 13, 3, "Province:"); ws.mergeCells(13, 4, 13, 5); setCell(ws, 13, 4, "Postal Code:");
  setCell(ws, 13, 6, "City:"); setCell(ws, 13, 8, "Province:"); ws.mergeCells(13, 10, 13, 11); setCell(ws, 13, 10, "Postal Code:");
  ...
  // Row 16-17: Property
  ws.mergeCells(16, 1, 17, 5); setCell(ws, 16, 1, `Property Name & Address: \n${data.property?.nameAddress || ""}`);
  ws.mergeCells(16, 6, 16, 9); setCell(ws, 16, 6, "Deal Status:", { bold: true });
  setCell(ws, 16, 10, "Conditional" + (data.dealStatus === "Conditional" ? " ✓" : ""));
  ws.mergeCells(17, 6, 17, 9); setCell(ws, 17, 6, "");
  setCell(ws, 17, 10, "Firm:" + (data.dealStatus === "Firm" ? " ✓" : ""));
  setCell(ws, 18, 1, "City:"); setCell(ws, 18, 3, "Province:"); setCell(ws, 18, 4, "Postal Code:");
  ws.mergeCells(18, 6, 18, 9); setCell(ws, 18, 6, "");
  setCell(ws, 18, 10, "Closed:" + (data.dealStatus === "Closed" ? " ✓" : ""));
  setCell(ws, 19, 1, data.property?.city || ""); setCell(ws, 19, 3, data.property?.province || "");
  ws.mergeCells(19, 6, 19, 9); setCell(ws, 19, 6, "Deal Closing Date:");
  ws.mergeCells(19, 10, 19, 11); setCell(ws, 19, 10, data.closingDate || "");
  ...
  ws.mergeCells(21, 4, 21, 5); setCell(ws, 21, 4, "Multi Housing" + (data.propertyType === "Multi Housing" ? " ✓" : ""));
  ws.mergeCells(21, 6, 21, 7); setCell(ws, 21, 6, "Interest Account?");
  ws.mergeCells(21, 8, 21, 9); setCell(ws, 21, 8, "Deposit Amount:");
  ws.mergeCells(21, 10, 21, 11); setCell(ws, 21, 10, data.depositAmount ?? "");
  ...
  ws.mergeCells(23, 6, 23, 8); setCell(ws, 23, 6, "Deduct for Commission?");
  setCell(ws, 23, 9, "Held By:");
  ws.mergeCells(23, 10, 23, 11); setCell(ws, 23, 10, "OTHER: " + (data.depositHeldBy || ""));

  // Row 24: Parcel size
  ws.mergeCells(24, 1, 24, 2); setCell(ws, 24, 1, "Parcel Size in Acres:");
  ws.mergeCells(24, 3, 24, 5); setCell(ws, 24, 3, data.parcelSizeAcres ? `${data.parcelSizeAcres} AC` : "");

  // Row 26-29: Building details
  ws.mergeCells(26, 1, 26, 2); setCell(ws, 26, 1, "Total Sq. Ft of Buildings");
  ws.mergeCells(26, 3, 26, 5); setCell(ws, 26, 3, data.buildingSF ?? 0);
  ws.mergeCells(27, 1, 27, 2); setCell(ws, 27, 1, "Number of Units");
  ws.mergeCells(27, 3, 27, 5); setCell(ws, 27, 3, data.numberOfUnits ?? 0);
  ws.mergeCells(28, 1, 28, 2); setCell(ws, 28, 1, "Portfolio");
  setCell(ws, 28, 3, "Yes" + (data.portfolio ? " ✓" : ""));
  setCell(ws, 28, 4, "No" + (!data.portfolio ? " ✓" : ""));
  ws.mergeCells(29, 1, 29, 2); setCell(ws, 29, 1, "Number of Buildings:");
  ws.mergeCells(29, 3, 29, 5); setCell(ws, 29, 3, data.numberOfBuildings ?? 0);

  // Occupier industry section (right side rows 26-39)
  ws.mergeCells(26, 6, 28, 11); setCell(ws, 26, 6, "OCCUPIER INDUSTRY:  ENTER THE APPROPRIATE OCCUPIER FROM THE OCCUPIER INDUSTRY TAB.", { bold: true });
  ws.mergeCells(29, 6, 32, 11); setCell(ws, 29, 6, data.occupierIndustry || "");

  // Row 30-32: CBRE Listing
  ws.mergeCells(30, 1, 30, 2); setCell(ws, 30, 1, "CBRE Listing?");
  setCell(ws, 30, 3, "Yes " + (data.cbreListing ? "✓" : ""));
  setCell(ws, 30, 4, "No " + (!data.cbreListing ? "✓" : ""));
  ws.mergeCells(31, 1, 31, 2); setCell(ws, 31, 1, "Type of Listing:");
  ...
  ws.mergeCells(33, 1, 33, 2); setCell(ws, 33, 1, "VENDOR SOLICITOR: ", { bold: true });
  ws.mergeCells(33, 3, 33, 5); setCell(ws, 33, 3, data.vendorSolicitor?.name || "");
  setCell(ws, 34, 1, "Contact Name: ");
  ws.mergeCells(34, 2, 34, 5); setCell(ws, 34, 2, data.vendorSolicitor?.contactName || "");
  setCell(ws, 35, 1, "Phone:  ");
  ws.mergeCells(35, 2, 35, 3); setCell(ws, 35, 2, data.vendorSolicitor?.phone || "");
  ws.mergeCells(36, 1, 37, 4); setCell(ws, 36, 1, `Mailing Address:\n${data.vendorSolicitor?.address || ""}`);
  ...
  ws.mergeCells(41, 3, 41, 5); setCell(ws, 41, 3, data.purchaserSolicitor?.name || "");
  setCell(ws, 41, 6, "OUTSIDE BROKER:", { bold: true });
  setCell(ws, 42, 1, "Contact Name:");
  ws.mergeCells(42, 2, 42, 5); setCell(ws, 42, 2, data.purchaserSolicitor?.contactName || "");
  setCell(ws, 42, 6, "Contact Name:");
  setCell(ws, 43, 1, "Phone:  ");
  ws.mergeCells(43, 2, 43, 3); setCell(ws, 43, 2, data.purchaserSolicitor?.phone || "");
  ws.mergeCells(44, 1, 45, 4); setCell(ws, 44, 1, `Mailing Address:\n${data.purchaserSolicitor?.address || ""}`);
  ...
  // OSB Commission
  ws.mergeCells(48, 6, 48, 9); setCell(ws, 48, 6, "OSB COMMISSION AMOUNT:");
  ws.mergeCells(48, 10, 48, 11); setCell(ws, 48, 10, 0);

  // Row 51: Commission Calculation
  setMergedLabel(ws, 51, 1, 11, "COMMISISON CALCULATION:", headerFill);

  // Row 52: Headers
  ws.mergeCells(52, 1, 52, 4); setCell(ws, 52, 1, "TOTAL PURCHASE PRICE:", { bold: true });
  ws.mergeCells(52, 5, 52, 7); setCell(ws, 52, 5, "Percentage Based on Sale Price", { bold: true });
  ws.mergeCells(52, 8, 52, 11); setCell(ws, 52, 8, "Total Commission", { bold: true });

  // Row 53: Values
  ws.mergeCells(53, 1, 53, 4); setCell(ws, 53, 1, data.purchasePrice ?? "");
  ws.mergeCells(53, 5, 53, 7); setCell(ws, 53, 5, data.commissionType === "setFee" ? "Set Fee Agreement" : (data.commissionPercentage || ""));
  ws.mergeCells(53, 8, 53, 11); setCell(ws, 53, 8, data.commissionSetFee ?? "");

  // Row 54: percentage calc
  if (data.commissionType === "percentage" && data.commissionPercentage) {
    ws.mergeCells(54, 1, 54, 4); setCell(ws, 54, 1, "");
    ws.mergeCells(54, 5, 54, 7); setCell(ws, 54, 5, data.commissionPercentage);
    ws.mergeCells(54, 8, 54, 11); setCell(ws, 54, 8, { formula: `A53*E54` } as unknown as number);
  }

  // Row 55: Total
  ws.mergeCells(55, 8, 55, 11);
  ...
  ws.mergeCells(58, 2, 58, 4); setCell(ws, 58, 2, "Branch Name:", { bold: true });
  ws.mergeCells(58, 5, 58, 6); setCell(ws, 58, 5, "Sales Person", { bold: true });
  ws.mergeCells(58, 7, 58, 8); setCell(ws, 58, 7, "Initials", { bold: true });
  ws.mergeCells(58, 9, 58, 11); setCell(ws, 58, 9, "Total Commission Due", { bold: true });
  ...
  ws.mergeCells(currentRow, 2, currentRow, 4); setCell(ws, currentRow, 2, offices[0]?.name || "Saskatoon");
  ws.mergeCells(currentRow, 9, currentRow, 11);
  ...
  splits.forEach((s: { name: string; pct: number }) => {
    ws.mergeCells(currentRow, 2, currentRow, 4); setCell(ws, currentRow, 2, offices[0]?.name || "Saskatoon");
    ws.mergeCells(currentRow, 5, currentRow, 6); setCell(ws, currentRow, 5, s.name);
    ws.mergeCells(currentRow, 9, currentRow, 11);
  ...
  for (let o = 1; o < offices.length; o++) {
    ...
    ws.mergeCells(currentRow, 2, currentRow, 4); setCell(ws, currentRow, 2, offices[o].name);
    ws.mergeCells(currentRow, 9, currentRow, 11);
    ...
    if (offices[o].people) {
      for (const p of offices[o].people) {
        ws.mergeCells(currentRow, 2, currentRow, 4); setCell(ws, currentRow, 2, offices[o].name);
        ws.mergeCells(currentRow, 5, currentRow, 6); setCell(ws, currentRow, 5, p.name);
        ws.mergeCells(currentRow, 9, currentRow, 11);
  ...
  // Invoice section
  currentRow += 2;
  ws.mergeCells(currentRow, 1, currentRow, 4); setCell(ws, currentRow, 1, "Send Invoice To:", { bold: true });
  currentRow++;
  setCell(ws, currentRow, 1, "Vendor" + (data.invoiceRecipient === "Vendor" ? " ✓" : ""));
  ws.mergeCells(currentRow, 3, currentRow, 4); setCell(ws, currentRow, 3, "Vendor's Lawyer" + (data.invoiceRecipient === "Vendor's Lawyer" ? " ✓" : ""));
  currentRow++;
  setCell(ws, currentRow, 1, "Purchaser" + (data.invoiceRecipient === "Purchaser" ? " ✓" : ""));
  ws.mergeCells(currentRow, 3, currentRow, 4); setCell(ws, currentRow, 3, "Purchaser's Lawyer" + (data.invoiceRecipient === "Purchaser's Lawyer" ? " ✓" : ""));
  currentRow++;
  setCell(ws, currentRow, 1, "Outside Broker" + (data.invoiceRecipient === "Outside Broker" ? " ✓" : ""));

  // Broker + GST note
  currentRow++;
  ws.mergeCells(currentRow, 7, currentRow, 11); setCell(ws, currentRow, 7, `Broker:  ${splits[0]?.name || "Michael Bratvold"}`);
  currentRow++;
  ws.mergeCells(currentRow, 1, currentRow, 5); setCell(ws, currentRow, 1, "Include both GST  & PST");
