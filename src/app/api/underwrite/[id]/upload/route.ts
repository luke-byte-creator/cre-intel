...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to parse upload. File may be too large or connection was interrupted." }, { status: 400 });
    }

    const file = formData.get("file") as File | null;
    if (!file) return NextResponse.json({ error: "No file provided" }, { status: 400 });

    console.log(`[upload] Received file: ${file.name} (${(file.size / 1024 / 1024).toFixed(1)}MB)`);
