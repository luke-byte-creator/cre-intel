            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th className="cursor-pointer hover:text-white text-right" onClick={() => toggleSort("net_rent_psf")}>Rent/SF<SortIcon col="net_rent_psf" /></th>
                  <th className="cursor-pointer hover:text-white text-right" onClick={() => toggleSort("area_sf")}>Size<SortIcon col="area_sf" /></th>
                  <th className="cursor-pointer hover:text-white" onClick={() => toggleSort("lease_start")}>Start<SortIcon col="lease_start" /></th>
  ...
  return (
    <>
      <tr className={`${isSelected ? "!bg-blue-500/10" : ""} cursor-pointer transition-colors`}>
        ...
        <td className="text-muted max-w-[180px] truncate" onClick={onToggle}>{r.tenant || "—"}</td>
        <td className="text-muted max-w-[180px] truncate" onClick={onToggle}>{r.landlord || "—"}</td>
        <td onClick={onToggle}><span className="px-2 py-0.5 rounded text-xs bg-card-border text-foreground/80">{r.propertyType || "—"}</span></td>
        <td className="text-right font-mono text-foreground" onClick={onToggle}>{fmtRent(r.netRentPSF)}</td>
        <td className="text-right text-muted" onClick={onToggle}>{fmtSF(r.areaSF)}</td>
        <td className="text-muted" onClick={onToggle}>{fmtD(r.leaseStart)}</td>
