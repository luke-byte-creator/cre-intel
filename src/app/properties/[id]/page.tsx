    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted text-sm mt-1">{pr.propertyType as string} · {pr.city as string}{pr.neighbourhood ? ` · ${pr.neighbourhood}` : ""}</p>
        </div>
        <WatchToggle entityType="property" entityId={Number(id)} entityLabel={label} initialWatched={watchStatus.watched} watchId={watchStatus.watchId} />
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-card-border">
                  ...
                  <th className="px-4 py-3 text-left text-muted font-medium">Date</th>
                  <th className="px-4 py-3 text-left text-muted font-medium">Work Type</th>
